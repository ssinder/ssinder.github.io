...
const DataManager = (function() {
    ...
    async function loadData() {
        ...
        if (cached && cached.p && cached.i) {
            console.log('[DEBUG] 从 IndexedDB 缓存加载数据 (complete格式)');
            proteinData = cached;
            buildIndex(cached);
            
            const endTime = performance.now();
            console.log(`[DEBUG] 数据加载完成 (缓存): ${cached.p.length} 个蛋白, ${cached.i ? cached.i.length : 0} 条互作, 耗时: ${(endTime - startTime).toFixed(2)}ms`);
            return cached;
        }

        // 从网络加载 - 优先尝试 complete 版本
        console.log('[DEBUG] 从网络加载数据...');
        
        // 首先尝试加载 complete 版本
        let response;
        try {
            response = await fetch('data/protein_interactions_complete.json');
            if (response.ok) {
                console.log('[DEBUG] 加载 protein_interactions_complete.json 成功');
            }
        } catch (e) {
            console.log('[DEBUG] 加载 complete 版本失败，尝试旧版本:', e.message);
            response = null;
        }
        
        // 如果 complete 失败，尝试旧版本
        if (!response || !response.ok) {
            try {
                response = await fetch('data/protein_interactions.json');
            } catch (e) {
                throw new Error('无法加载任何数据文件');
            }
        }
